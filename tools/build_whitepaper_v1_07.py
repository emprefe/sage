...
def build():
    ...
    story = [
        ...
        Spacer(1, 0.35 * inch),
        p("Status: proposed design revision. The repository implementation remains the authoritative record of what is currently implemented. This paper consolidates the intended direction; it does not silently change the v0.01 wire format.", note),
        Spacer(1, 0.5 * inch),
        ...
        p("The exact v1.07 wire syntax must be frozen with conformance vectors before implementation changes are made. The conceptual form below illustrates the intended semantics; it is not yet a replacement for the repository's current v0.01 grammar.", note),
        p("SAGE/1.07|PARTICIPANT_ENTRY|PARTICIPANT_ENTRY|...", code),
        p("A normative revision should define UTF-8 encoding, identifier bounds, extension presence and length limits, delimiter escaping or binary framing, canonical equality, malformed-record behavior, and compatibility with earlier versions.", body),
        p("The implementation should retain a versioned parser so v0.01 records can be recognized or rejected explicitly rather than being misinterpreted as v1.07 records.", body),
        ...
        p("The repository currently contains a Python-first v0.01 reference implementation with Core parsing, PNG metadata transport, an experimental concealed PNG profile, CLI commands, conformance vectors, transformation fixtures, and CI.", body),
        ...
        p("The current code still uses the earlier AI_ID / generation-ID Core shape. It must be updated only after the v1.07 participant-entry grammar and compatibility policy are frozen.", note),
        PageBreak(),
        ...
        table([
            ...
            [p("B. Python oracle", cell), p("Update Core, profiles, CLI, vectors, and tests; retain explicit v0.01 compatibility behavior.", cell)],
